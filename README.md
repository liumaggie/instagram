...
<img src="https://github.com/liumaggie/instagram/blob/master/app/assets/images/auth-form.png" alt="AuthForm" width="80%" height="auto">
...
These images load by infinite scroll, where an AJAX call is made for additional images to render when the bottom of the page is reached. The images of who the current user is following (`followees`) are then prefetched using the data passed into `params` from the AJAX call as shown below:
...
<img src="https://github.com/liumaggie/instagram/blob/master/app/assets/images/current-user-profile-page.png" alt="UserProfile" width="80%" height="auto">
...
In addition to the features already implemented, I plan to continue working on the following features for this project.
...
### Notifications
Notifications is another important feature for Instagram. It would be beneficial for users to be notified after a user follows another user or likes/comments on an image. I would use WebRTC for notifications to appear for the users.

### Direct Messaging
Messaging between users is an interesting feature that would be fun to implement since users can directly interact with other users real-time. I would also use WebRTC for this feature.
